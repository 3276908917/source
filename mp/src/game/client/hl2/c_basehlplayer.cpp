...
// Purpose: Appears to handle most movement and collision detection #L
// $NoKeywords: $
// FPC
...
// How fast to avoid collisions with center of other object, in units per second #V
static const float AVOID_SPEED = 2000.0f;
...
// Purpose: ?
void C_BaseHLPlayer::Weapon_DropPrimary( void ) { engine->ServerCmd( "DropPrimary" ); }

float C_BaseHLPlayer::GetFOV() {
	//Find our FOV with offset zoom value #V
	float flFOVOffset = BaseClass::GetFOV() + GetZoom();
	// Clamp FOV in MP #V
	int min_fov = ( gpGlobals->maxClients == 1 ) ? 5 : default_fov.GetInt();
	// Don't let it go too low #V
...
float C_BaseHLPlayer::GetZoom( void ) {
	...
	if ( ( m_flZoomStart != m_flZoomEnd ) && ( m_flZoomRate > 0.0f ) ) {
		...
		if ( deltaTime >= 1.0f ) {
			//If we're past the zoom time, just take the new value and stop lerping #V
			fFOV = m_flZoomStart = m_flZoomEnd;
		}
		else fFOV = SimpleSplineRemapVal( deltaTime, 0.0f, 1.0f, m_flZoomStart, m_flZoomEnd );
	}
	return fFOV;
}
...
bool C_BaseHLPlayer::TestMove( const Vector &pos, float fVertDist, float radius, const Vector &objPos, const Vector &objDir ) {
	...
	if ( trOver.fraction < 1.0f ) {
		// check if the endpos intersects with the direction the object is travelling. If it doesn't, this is a good direction to move. #V
		if ( objDir.IsZero() ||
			( IntersectInfiniteRayWithSphere( objPos, objDir, trOver.endpos, radius, &flHit1, &flHit2 ) && 
			( ( flHit1 >= 0.0f ) || ( flHit2 >= 0.0f ) ) )
			) {
			// our first trace failed, so see if we can go farther if we step up. #V

			// trace up to see if we have enough room. #V
			...
			// do a trace from the stepped up height #V
	...
	// trace down to see if this position is on the ground #V
...
// Client-side obstacle avoidance #V
void C_BaseHLPlayer::PerformClientSideObstacleAvoidance( float flFrameTime, CUserCmd *pCmd )
{
	...
	// This should perhaps rely on a global constant #L
	if ( curspeed > 150.0f ) {
	...
	if ( fabs( pCmd->forwardmove ) > 0.0f || fabs( pCmd->sidemove ) > 0.0f ) {
		istryingtomove = true;
		if ( pCmd->forwardmove > 1.0f ) ismovingforward = true;
	}
	...
	// Okay, decide how to avoid if there's anything close by #V
	...
	for ( int i = 0; i < c; i++ ) {
		...
		// Figure out a 2D radius for the object #V
		...
		//Don't run this code if the NPC is not moving UNLESS we are in stuck inside of it. #V
		if ( !obj->IsMoving() && flDist > objectradius ) continue;

		if ( flDist > objectradius && obj->IsEffectActive( EF_NODRAW ) ) obj->RemoveEffects( EF_NODRAW );
		...
		if ( !IntersectInfiniteRayWithSphere( GetAbsOrigin(), vRayDir, obj->GetAbsOrigin(), radius, &flHit1, &flHit2 ) ) continue;
		...
		if ( flNPCSpeed > 0.001f ) {
			...
			// Start with whatever side they're on relative to the NPC's velocity. #V
			...
			for ( int nTries = 0; nTries < 2; nTries++ ) {
				...
				if ( TestMove( vTestPosition, size.z * 2, radius * 2, obj->GetAbsOrigin(), vMoveDir ) ) {
					...
					// Try the other direction. #V
					iDirection *= -1;
				}
			}
		} else {
			// the object isn't moving, so try moving opposite the way it's facing #V
			...
			if ( TestMove( vTestPosition, size.z * 2, radius * 2, obj->GetAbsOrigin(), vMoveDir ) ) {
				...
				if ( flDist < objectradius ) obj->AddEffects( EF_NODRAW );
		...
		if ( !foundResult ) {
			// test if we can move in the direction the object is moving #V
			...
			if ( TestMove( vTestPosition, size.z * 2, radius * 2, obj->GetAbsOrigin(), vMoveDir ) ) {
				...
				// try moving directly away from the object #V
		...
		if ( !foundResult ) {
			// test if we can move through the object #V
			Vector vTestPosition = GetAbsOrigin() - vMoveDir * radius * 2;
			fwd = currentdir.Dot( -vMoveDir );
			rt = rightdir.Dot( -vMoveDir );
		...
		// If running, then do a lot more sideways veer since we're not going to do anything to forward velocity #V
		if ( istryingtomove ) sidescale = 6.0f;

		if ( flVelProduct > 0.0f && flDirProduct > 0.0f ) sidescale = 0.1f;
	...
	// complicated because we have different limits for forward, back, and side #V
...
// Though I am not sure how there would ever be server-based NPCs #L
	...
	if ( m_hClosestNPC == NULL ) {
		...
		if ( pNPC ) {
			...
			if ( flDist < pNPC->GetSpeedModifyRadius() ) {
				...
				if ( flDot > 0.8 ) {
					/*
					// Velocity check (abort if the target isn't moving) #V
